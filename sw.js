...
self.addEventListener('fetch',e=>{if(e.request.url.includes('google.com/vt')||e.request.url.includes('dapi.kakao'))return;
  e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).then(res=>{if(res&&res.status===200&&res.type!=='opaque'){var c=res.clone();caches.open(CACHE).then(ca=>ca.put(e.request,c));}return res;}).catch(()=>caches.match('./index.html'))));});
